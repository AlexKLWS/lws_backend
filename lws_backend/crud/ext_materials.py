...
def get_ext_material_previews(
...
) -> List[ExtMaterialPreview]:
    ext_material_preview_filter = None
    if page_index.page == 1:
        ext_material_preview_filter = (
            and_(
                ExtMaterialPreview.created_at >= page_index.end_date,
                ExtMaterialPreview.category == category.value,
            )
            if category != Category.MISC
            else and_(ExtMaterialPreview.created_at >= page_index.end_date)
        )
    else:
        ext_material_preview_filter = (
            and_(
                ExtMaterialPreview.created_at >= page_index.end_date,
                ExtMaterialPreview.created_at <= page_index.start_date,
                ExtMaterialPreview.category == category.value,
            )
            if category != Category.MISC
            else and_(
    ...
    if ext_material_preview_filter is not None:
        ext_material_previews = (
            db.query(ExtMaterialPreview).filter(ext_material_preview_filter).all()
        )
